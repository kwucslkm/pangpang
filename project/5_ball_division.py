...
balls.append({
    ...
    "to_y" : -6, # y 축 이동 방향
    "init_spd_y" : ball_speed_y[0] # 공의 y 방향 속도
})
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        ball_img_idx = ball_val["img_idx"]
        # 공 rect 정보 업데이트
        ball_size = ball_images[ball_img_idx].get_rect()
        ball_size.left = ball_pos_x
        ball_size.top = ball_pos_y
        # 공과 캐릭터 충돌 처리
        if character_rect.colliderect(ball_size):
            print("캐릭터 공에 충돌")
            running = False
            break
        # 공과 무기들 충돌 처리
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            # 충돌 체크
            if weapon_rect.colliderect(ball_size):
                print("공과 무기 충돌")
